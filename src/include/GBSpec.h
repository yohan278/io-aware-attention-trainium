...
namespace spec {
  namespace GB {
    namespace Large {
      ...
      const unsigned int kEntriesPerBank =
          4096; // total global buffer size = 4096*16banks*16scalars*8bits = 8Mb
                // = 1MB
      ...
      public:
        ...
        static const unsigned int width =
            nvhls::nv_scvector<WordType, N>::width;
        template <unsigned int Size>
        void Marshall(Marshaller<Size>& m) {
          m & read_vector;
        }
        DataRsp() { Reset(); }
        void Reset() { read_vector = 0; }
      };
    } // namespace Large
  } // namespace GB
} // namespace spec
